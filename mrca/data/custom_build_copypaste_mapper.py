...
from detectron2.data.datasets.coco import load_coco_json, load_coco_syn_json
from detectron2.data.dataset_mapper import DatasetMapper
from detectron2.data import detection_utils as utils
from detectron2.data import transforms as T
import detectron2.utils.comm as comm
from detectron2.structures import BitMasks, Boxes, Instances
from mrca.data.transforms.custom_cp_method import blend_image
import sys
sys.path.append('tools')
from mrca.data.dataset_mapper import DatasetMapper
...
class InstPoolFeed:
    def __init__(self, json_file, image_root, train_size, max_samples=20, image_format="BGR",use_largest_part=True,random_rotate=False,cp_method=['basic'],color_aug=False,transition_matrix_path='',active_select=False, cfg = None, rd=None, cat_pool=None) -> None:
        self.max_gpu_memory = get_gpu_memory_usage()
        
        pascal = False
        oiv5 = False
        if "pascal" in image_root:
            pascal = True
        elif "oiv5" in image_root:
            oiv5 = True
        
        if image_format=='RGBA':
            
            json_path = json_file + '/rd' + str(rd) + '_annotations.json'
            json_done_path = json_file + '/rd' + str(rd) + 'done.txt'
            print("Wait for {}".format(json_done_path))
            while not os.path.exists(json_done_path):
                time.sleep(1)
            ...
            # for adapative-size pasting 
            
            val_path = ''  # set validation folder path
    
            if rd > 1:
                idxt = (rd-2)*2 # use bbox accuracy
                path = val_path + '/per_class_mAP{}.json'.format(idxt)
                with open(path) as f:
                    self.clsAP=list(json.load(f).values())
                self.map_value = np.nanmean(self.clsAP)
            else:
                self.clsAP=None
                self.map_value=None
            ...
            self.dataset = load_coco_syn_json(json_path, image_root)
            self._get_per_cat_pool()
            self.idx_to_cat = {i:cat for cat in self.per_cat_pool for i in self.per_cat_pool[cat]}
        else:
            self.dataset = load_coco_syn_json(json_path, image_root)
        ...
        if self.active_select:
            ...
            self.per_cat_pool_real = None
            self.non_empty_cat = None
        if cat_pool is not None:
    
            self.per_cat_pool_real = cat_pool
            self.non_empty_cat = self.set_non_empty_catpool(cat_pool)
        
        self.cfg = cfg

        self.area_std_thres = cfg.INPUT.INST_POOL_AREA_STD_THRES
        self.area_certainty = cfg.INPUT.INST_POOL_AREA_CERTAINTY
        
        # for uniform object sampling
        self._init_uniform_sample_pool()

    def _init_uniform_sample_pool(self):
        self.uniform_sample_pool = list(range(len(self.dataset)))
        np.random.shuffle(self.uniform_sample_pool)


    def set_non_empty_catpool(self, per_cat_pool):
        non_empty_catpool = []
        for select_class in range(0, 1203):
            if len(per_cat_pool[select_class]) != 0:
                non_empty_catpool.append(select_class)
        
        return non_empty_catpool
               
        
    # create per_cat_pool  (type: dict)
    #    input: category id
    #    output: index in self.dataset
    ...
    # get mixed result  (type: dict)
    #    input: category id
    #    output: results_origin
    #               - holds pasted image info


    # reference : CopyPaste (paster)
    # data_dict: self.mapper(dataset_dict)

    # currently called with "cids=None,data_dict=None,reference=None"

    # number of samples chosen here
    ...
    def get_mix_result(self, sample_type='random', cids=None,data_dict=None,reference=None):
        ...
        # if reference given, create new image based on it
        ...
        # else 
        # 1. randomly choose number of samples
        # 2. get samples based on different strategies
        # 3. create new image based on those chosen samples(ids) by _cat_a_new_image
            ...
            if sample_type == 'random':
                ...
                if len(cids) ==0:
                    cids = None
                ids = self._get_cls_balanced_random_samples(num_sample, cids=cids)
            elif sample_type == 'one':
                num_sample = 1
                num_sample = np.random.randint(1, 3)
                ids = self._get_random_samples(num_sample)
            elif sample_type == 'uniform':
                num_sample = np.random.randint(1, 3)
                ids = self._get_uniform_samples(num_sample)
            else :
        ...
        # Save results to results_origin
        ...
        # paste_class_set = set(results_origin['instances'].gt_classes.tolist()) - set(label_dict)
        # paste_class_set = set([self.idx_to_cat[i] for i in ids])
        paste_class_set = set(self.non_empty_cat)
        #print('paste_class_set:',paste_class_set)
        if self.active_select:
            # select image from paste_class_set
            if self.cfg.MODEL.ACTIVE_TEST == 'select':
    ...
    # Lot of functions for getting dataset indexes for sampling    


    # # Our method for choosing number of samples (already well-generated, just choose number to paste all in necessary time)
    # def _get_all_samples(self, nums, cids = None):
    ...
    def _get_uniform_samples(self, nums):
        """
        Get uniformly sampled objects from the dataset.
        """
        if len(self.uniform_sample_pool) < nums:
            self._init_uniform_sample_pool()
        idxs = [self.uniform_sample_pool.pop() for _ in range(nums)]
        return idxs
    ...
    # size related info here
    #   - target_WH
    def compute_scale_factor(self, ap, map_value):
        """
        Compute the scale factor based on AP with constraints:
        1. ap = 0   -> scale factor = 16
        2. ap = map -> scale factor = 1
        3. ap = 100 -> scale factor = 1/16
        
        Args:
            ap (float): Average Precision (0 to 100)
            map_value (float): Mean Average Precision (0 to 100)
        ...
        # Map AP values to log scale for interpolation
        log_scale_min = np.log(16)      # log scale at ap = 0
        log_scale_max = np.log(1/16)    # log scale at ap = 100
        log_scale_mid = np.log(1)       # log scale at ap = map_value (1)

        # Use piecewise linear interpolation in log scale
        if ap < map_value:
            # Interpolate between (0, log_scale_min) and (map, log_scale_mid)
            scale = log_scale_min + (log_scale_mid - log_scale_min) * (ap / map_value)
        else:
            # Interpolate between (map, log_scale_mid) and (100, log_scale_max)
            scale = log_scale_mid + (log_scale_max - log_scale_mid) * ((ap - map_value) / (100 - map_value))
            # return 1
    ...
    def _load_RGBA(self,img_path,train_size,target_WH=None):
        ...
        image_H,image_W=train_size
        # print(img_path)
        # print(img_path['file_name'][-8:-4])
        # label=self.data_to_cat[img_path] # 0~1202
        label= int(img_path['file_name'][-13:-9]) -1
        # mask_path=None
        # if img_path[0]=='*':
        img_path=img_path['file_name']
        mask_path=img_path.replace("raw","segmented")
        #print(self.max_gpu_memory)
        ...
        if self.clsAP is not None:
            ap = self.clsAP[label]
            scale_factor = self.compute_scale_factor(ap, self.map_value)

            area_mean = area_mean * scale_factor
        ...
        # 2. resize based on target WH, and get segmentation
        ...
        # 3. apply augmentation and return object segmentation information
    ...
    # Copy paste data with given reference
...
class CopyPasteMapper:
    def __init__(self, mapper, cfg, dataset=None, rd=None):
        self.mapper = mapper
        if dataset is None :
            repeat_probs = None
        else :    
        ...
        if cfg.INPUT.INST_POOL :
            ...
            self.inst_pool = None
        if cfg.INPUT.INST_POOL_FEED :
            self.inst_pool = InstPoolFeed(cfg.INPUT.INST_POOL_PATH, cfg.INPUT.INST_POOL_ROOT, cfg.INPUT.TRAIN_SIZE, image_format=cfg.INPUT.INST_POOL_FORMAT, max_samples=cfg.INPUT.INST_POOL_MAX_SAMPLES,
                                      use_largest_part=cfg.USE_LARGEST_PART,random_rotate=cfg.INPUT.RANDOM_ROTATE,cp_method=cfg.INPUT.CP_METHOD,color_aug=self.src_color, transition_matrix_path = cfg.INPUT.TRANSITION_MATRIX_PATH,
                                      active_select = cfg.INPUT.ACTIVE_SELECT, cfg = cfg, rd = rd)
            self.inst_pool.cfg = cfg
            self.inst_pool_sample_type = cfg.INPUT.INST_POOL_SAMPLE_TYPE
            self.inst_pool.area_std_thres = cfg.INPUT.INST_POOL_AREA_STD_THRES
            self.inst_pool.area_certainty = cfg.INPUT.INST_POOL_AREA_CERTAINTY
        ...
        if cfg.INPUT.LOG_SCP_PARAM :
            ...
            print("saving to {}".format(str(self.img_save_dir)))
        ...
        if self.active_select :
                if dataset is None:
                    if self.inst_pool.per_cat_pool_real is None :
                        ...
                        self.inst_pool.non_empty_cat = self.set_non_empty_catpool(self.per_cat_pool_real)
                else :
