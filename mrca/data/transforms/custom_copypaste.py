...
from odod.data.transforms.custom_cp_method import blend_image
import math
import json
import cv2
import os
...
class CopyPaste:
    ...
    def _inp_rotate(self, results, inp=True):
        ...
        save_img = 0 # set 1 to save augmented images
        imagesavepath = 'imagesavepath/' # set the path to save augmented images
        if save_img :
            ...
            vis.save(imagesavepath + '{}_show.jpg'.format(self.count))
            self.count += 1 
    ...
    def __call__(self, results, logger=None, save_img_dir=None):
        ...
        if save_img_dir is not None :
            ...
            print("write image to: {}".format(save_img_dir))
        h, w = results['img'].shape[-2:]
        results_origin['image'] = torch.from_numpy(results['img'])
